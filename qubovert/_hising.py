...
from .utils import BO, HIsingMatrix, IsingMatrix, solve_hising_bruteforce
...
class HIsing(BO, HIsingMatrix):
    ...
    def __init__(self, *args, **kwargs):
        ...
        """
        # Call HIsingMatrix.__init__.  We include a method here just so we can
    ...
    def to_hising(self):
        ...
        Create and return upper triangular HIsing representing the problem.
    ...
    def to_ising(self, lam=1):
        ...
        lam : float (optional, defaults to 1).
            ...
            constraints. When we reduce the degree of the HIsing to a Ising, we
            add penalties to the Ising in order to enforce ancilla variable
        ...
        try:
            ...
            raise NotImplementedError("``to_ising`` not implemented yet!")
    ...
    def convert_solution(self, solution):
        ...
        if len(solution) == self.num_binary_variables:
            ...
            raise NotImplementedError("HIsing ising and qubo functionality "
                                      "not done!")
    ...
    def solve_bruteforce(self, *args, **kwargs):
        ...
        PROBLEMS! It converts the problem to HIsing, solves it with
        ``qubovert.utils.solve_hising_bruteforce``, and then calls and returns
        ``convert_solution``.

        Parameters
        ----------
        *args and **kwargs : arguments and keyword arguments.
            Contains args and kwargs for the ``to_hising`` method. Also
            contains a ``all_solutions`` boolean flag, which indicates whether
            or not to return all the solutions, or just the best one found.
        ...
        hising = self.to_hising(*args, **kwargs)
        _, sol = solve_hising_bruteforce(hising, all_solutions=all_solutions)
        if all_solutions:
